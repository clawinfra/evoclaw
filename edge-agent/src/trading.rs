...
pub struct HyperliquidClient {
    ...
    asset_index: Arc<Mutex<HashMap<String, u32>>>,
}
...
impl HyperliquidClient {
    pub fn new(config: TradingConfig) -> Self {
        ...
        Self {
            ...
            private_key,
            rate_limiter,
            asset_index: Arc::new(Mutex::new(HashMap::new())),
        }
    }

    async fn resolve_asset(&self, coin: &str) -> Result<u32, Box<dyn std::error::Error>> {
        {
            let idx = self.asset_index.lock().await;
            if let Some(&i) = idx.get(coin) {
                return Ok(i);
            }
        }
        ...
        self.asset_index
            ...
            .get(coin)
            .copied()
            .ok_or_else(|| format!("unknown asset: {}", coin).into())
    }
    ...
    pub async fn get_prices(&self) -> Result<HashMap<String, f64>, Box<dyn std::error::Error>> {
        ...
        Ok(r.mids
            .into_iter()
            .filter_map(|(c, p)| p.parse().ok().map(|v| (c, v)))
            .collect())
    }
    ...
    pub async fn get_open_orders(&self) -> Result<Vec<OpenOrder>, Box<dyn std::error::Error>> {
        ...
        Ok(self
            .client
            .post(&url)
            .json(&serde_json::json!({"type": "openOrders", "user": self.config.wallet_address}))
            .send()
            .await?
            .json()
            .await?)
    ...
    pub async fn get_fills(&self) -> Result<Vec<Fill>, Box<dyn std::error::Error>> {
        ...
        Ok(self
            .client
            .post(&url)
            .json(&serde_json::json!({"type": "userFills", "user": self.config.wallet_address}))
            .send()
            .await?
            .json()
            .await?)
    ...
    pub async fn cancel_all_orders(
        ...
        for order in orders {
                ...
                .await
            {
                Ok(r) => results.push(r),
                Err(e) => {
                    warn!(oid = order.oid, error = %e, "cancel failed");
                }
            }
        }
        Ok(results)
        ...
        for attempt in 0..3u32 {
            ...
            match self.client.post(&url).json(&body).send().await {
                Ok(resp) => {
                    ...
                    if st.is_success() {
                        return Ok(serde_json::from_str(&text).unwrap_or(OrderResponse {
                            status: "ok".to_string(),
                            response: serde_json::json!({"raw": text}),
                        }));
                    }
                    if st.as_u16() == 429 {
    ...
    pub async fn monitor_positions(
        ...
        let mut total = 0.0;
        for p in positions {
            if let Ok(u) = p.unrealized_pnl.parse::<f64>() {
                total += u;
                info!(coin = %p.coin, size = %p.szi, unrealized_pnl = u, "position update");
            }
        }
...
pub fn format_size(size: f64, decimals: u32) -> String {
...
mod tests {
    ...
    fn create_test_config() -> TradingConfig {
        TradingConfig {
            ...
            paper_log_path: "test_paper.jsonl".to_string(),
        }
    }
    ...
    #[test]
    fn test_cancel_order_req() {
        let r = CancelOrderRequest {
            coin: "ETH".to_string(),
            oid: 12345,
        };
        assert_eq!(r.oid, 12345);
    }
    #[test]
    fn test_modify_order_req() {
        let r = ModifyOrderRequest {
            oid: 999,
            coin: "BTC".to_string(),
            is_buy: false,
    ...
    fn test_order_response() {
        let r: OrderResponse =
            serde_json::from_str(r#"{"status":"ok","response":{"oid":1}}"#).unwrap();
        assert_eq!(r.status, "ok");
    }
    #[test]
    fn test_position_deser() {
        let p: Position = serde_json::from_str(r#"{"coin":"BTC","szi":"0.5","entryPx":"50000","positionValue":"25000","unrealizedPnl":"500","returnOnEquity":"0.02"}"#).unwrap();
    ...
    fn test_pnl_serde() {
        ...
        t.record_trade(-30.0);
        t.update_unrealized(50.0);
        let j = serde_json::to_string(&t).unwrap();
        let d: PnLTracker = serde_json::from_str(&j).unwrap();
        assert_eq!(d.total_pnl, 120.0);
    }
    #[test]
    fn test_all_mids() {
        let r: AllMidsResponse =
            serde_json::from_str(r#"{"mids":{"BTC":"50000","ETH":"3000"}}"#).unwrap();
        assert_eq!(r.mids.len(), 2);
    }
    #[test]
    fn test_format_price() {
        assert_eq!(format_price(50000.0), "50000.0");
        assert_eq!(format_price(3000.5), "3000.50");
    }
    #[test]
    fn test_format_size() {
        assert_eq!(format_size(0.01, 2), "0.01");
    }
    #[test]
    fn test_tif_serde() {
        let j = serde_json::to_string(&TimeInForce::Gtc).unwrap();
        assert_eq!(
            serde_json::from_str::<TimeInForce>(&j).unwrap(),
            TimeInForce::Gtc
        );
    }
    #[test]
    fn test_open_order() {
        let o: OpenOrder = serde_json::from_str(r#"{"coin":"BTC","oid":1,"limitPx":"50000","sz":"0.01","side":"B","timestamp":1700000000}"#).unwrap();
        assert_eq!(o.oid, 1);
    }
    #[test]
    fn test_fill() {
        let f: Fill = serde_json::from_str(r#"{"coin":"BTC","px":"50000","sz":"0.01","side":"B","time":1700000000,"fee":"0.5","oid":1,"closedPnl":"100"}"#).unwrap();
        assert_eq!(f.closed_pnl, "100");
    }
    #[tokio::test]
    async fn test_rate_limiter() {
        let l = RateLimiter::new(5, Duration::from_secs(1));
        for _ in 0..5 {
            l.acquire().await;
        }
    }
    ...
    async fn test_rate_limiter_blocks() {
        let l = RateLimiter::new(2, Duration::from_millis(100));
        let s = Instant::now();
        for _ in 0..3 {
            l.acquire().await;
        }
        assert!(s.elapsed() >= Duration::from_millis(80));
    }
    #[test]
    fn test_timestamp() {
    ...
    fn test_meta() {
        ...
        assert_eq!(m.universe[0].name, "BTC");
    }
}
