...
impl SkillRegistry {
    ...
    pub fn load_from_config(&mut self, config: &SkillsConfig) {
        if let Some(cc) = &config.clawchain {
            if !cc.enabled {
                self.enabled.insert("clawchain".to_string(), false);
            }
        }
    }
...
mod tests {
    ...
    async fn test_init_all_with_failure() {
        ...
        assert!(!registry.is_enabled("bad_skill"));
    }
    ...
    fn test_load_from_config_disables() {
        let mut registry = SkillRegistry::new();
        registry.register(Box::new(MockSkill::new("clawchain")));

        let config = SkillsConfig {
            clawchain: Some(crate::config::ClawChainSkillConfig {
                enabled: false,
        ...
        assert!(!registry.is_enabled("clawchain"));
    }

    #[test]
    fn test_load_from_config_enabled() {
        let mut registry = SkillRegistry::new();
        registry.register(Box::new(MockSkill::new("clawchain")));
    ...
    #[test]
    fn test_load_from_config_none() {
        let mut registry = SkillRegistry::new();
        registry.register(Box::new(MockSkill::new("clawchain")));

        let config = SkillsConfig { clawchain: None };
        registry.load_from_config(&config);

        // Should remain enabled (default)
        assert!(registry.is_enabled("clawchain"));
    }
